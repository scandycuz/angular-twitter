...
class TwitterRequest
  ...
  def update_tweets
    ...
    current_group = Group.all.first
    current_group_number = current_group.number + 1
    current_group_number = current_group_number > total_groups ? 1 : current_group_number ;
    current_group.number = current_group_number
    current_group.save

    prominent_handles = Handle.where("handles.group = ?", current_group_number)
